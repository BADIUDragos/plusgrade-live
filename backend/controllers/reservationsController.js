...
exports.getReservations = async (req, res) => {
  try {
    const productAssignments = await fs.readJson(
      path.join(__dirname, "../product_assignment.json")
    );
    const productCharges = await fs.readJson(
      path.join(__dirname, "../product_charges.json")
    );
    ...
    productAssignments.forEach((assignment) => {
      ...
      const detailedProduct = {
        ...
        amount: productCharge
          ? Math.round((productCharge.amount + Number.EPSILON) * 100) / 100
          : null,
      };

      reservationDetails[reservation_uuid].reservationCharges.push(
        detailedProduct
      );

      if (productCharge && productCharge.active) {
        reservationDetails[reservation_uuid].numberOfActiveCharges += 1;
        reservationDetails[reservation_uuid].sumOfActiveCharges +=
          productCharge.amount || 0;
      }
    });

    for (const reservation of Object.values(reservationDetails)) {
      reservation.sumOfActiveCharges =
        Math.round((reservation.sumOfActiveCharges + Number.EPSILON) * 100) /
        100;
    }
